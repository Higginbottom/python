...
get_matom_f ()
{
  ...
  if (rank_global < np_mpi_global)
    {
      my_nmin = (rank_global * num_mpi_cells);
      my_nmax = ((rank_global + 1) * num_mpi_cells);
    }
  else
    {
      my_nmin = (rank_global * num_mpi_cells);
  ...
  Log
    ("!!JM: num_mpi_cells %d num_mpi_extra %d np_mpi_global %d NPLASMA %d ndo %d mynmin %d mynmax %d",
     num_mpi_cells, num_mpi_extra, np_mpi_global, NPLASMA, ndo, my_nmin,
     my_nmax);
#endif
