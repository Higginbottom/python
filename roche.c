...
/**************************************************************************
  ...
  Synopsis:
   ...
   These routines are used to describe the Roche geometry and are intended to allow one
  ...
  History:
   ...
   97jul   ksl	Coded
   01jul   ksl  Created a program plot_roche, which uses these subroutines
                Improved the notes on this.  Replace the pillbox routine so
		that it was simpler and more correct, especially for photons
		which were outside the L1 plane.  It is still true that photons
		which move directly along the x axis, and perhaps others may
		present problems.
...
/*
 * Given the mass of the primary and secondary and the period calculate some
 * other basic parameters for the binary system, including the L1 point.
 */

/*
 * p_roche provides a way to pass a photon structure to roche() and
 * roche_derive which are in turn used by the Numerical Recipes Routine
 * rtsafe to find the zero either to dphi_ds or to phi along the path lenght
 * of the photon
 */
...
binary_basics()
{
	...
	/*
	 * p_roche is used here to provide an external way to pass a photon
	 * structure to find_l1
	 */
	...
	geo.phi = phi(geo.l1);	/* Set geo.phi so phi will be zero on Roche
				 * lobes */


	/*
	 * Set geo.r2_far to be the radius of the secondary on the backside
	 * of the secondary
	 */
	...
	/*
	 * Calculate the maximum half width of the secondary in the plane of
	 * the orbit
	 */
...
/*
 * Find out whether a photon will hit the secondary, but don't actually
 * calculate the point where it hits
 */
...
/*
 * Find out whether the photon hits the pillbox which surrounds the Roche
 * surface of the secondary.
 * 
 * The pillbox is defined by a plane at L1, a plane on the backside of the star
 * and a cylinder whose radius is the maximum (half) width of the star in the
 * plane of the orbit.
 * 
 * This routine distinguishes two basic types of photons, "normal" and
 * "abnormal" photons.  A normal photon is on the WD side of L1 plane; an
 * abnormal one is on the side of the L1 plane with the secondary.
 * 
 * 
 * pillbox returns VERY_BIG if the photon p has not intersected the pillbox.  It
 * appears to return the distance to the pillbox if the photon p hits the
 * pillbox.  smin and smax contain the entrance and exit distances.  It is
 * not obvious that smin < smax.
 * 
 * Note:   01jul22	I revised this so that it would handle photons coming in all
 * directions for use with plot_roche but this instantiation of pillbox still
 * seems to me not optical.  The basic point is that unless the photon is in
 * the pillbox, the only roots that matter are ones which are positive.  Only
 * inside the pillbox will there be a positive and a negative root.  Here we
 * try to correct this at the end by making sure that both roots are not
 * negative, but one would have thought that one might establish that that
 * photons inside the pillbox are special and dealt with them accordindly
 * 
 * History:
 * 
 * 01jul	ksl	Completely revised in the process of creating plot_roche
 * 01jul22	ksl	Fixed a problem which allowed photons headed
 * completely away from the pillbox to be counted as hitting the pillbox.
 * See note above
 */
	...
	double         *smin, *smax;
{
	...
	n = 0;
	//At this point no boundaries to the pillbox have been identified

	/*
	 * If the photon is along the x-axis, then it will not intersect the
	 * cylinder anywhere, and the only possibility is that it hit the
	 * endcaps of the cylinder. But normally, it will not be along the x
	 * axis and one must find if there are any intersections with the
	 * cylinder and determine where they are.  This calculation is
	 * carried out in the if section below.
	 */
		...
		/*
		 * After "quadratic", root contains the distances the photon
		 * pp would need to travel to hit the edges of the cylinder.
		 * i is an index to the smallest positive root, if one
		 * exists.  If both of the roots are negative or imaginary
		 * then i will be negative, and we know that the ray did not
		 * hit the cylinder while travelling in the positive
		 * direction.  In that case return  VERY_BIG
		 * 
		 * Note that the fact that the ray does hit the cylinder going
		 * in the positive direction does not necessarily mean that
		 * it hits the pillbox.
		 */
		...
		/*
		 * If the intersection is in the part of the cylinder between
		 * the caps it is probably legitimate. So tranport pp to the
		 * intersection of the first root and check where that lies.
		 * And then do the same thing for the second root.  The
		 * "probably refers to the fact that a negative root is
		 * really only possible if the photon is already in the
		 * pillbox
		 */
		...
		stuff_phot(p, &pp);
		move_phot(&pp, root[1]);	/* So pp is now located at
	...
	/*
	 * Now find out if the photon hits the caps. This is done by
	 * translating to the planes of the two end caps and then checking
	 * the cylindrical radius. Negative distances are valid only if the
	 * photon is in the pillbox already
	 */

	x1 = ds_to_plane(&plane_l1, p);	/* Calculate the distance to the L1
					 * plane */
	stuff_phot(p, &pp);
	...
	x2 = ds_to_plane(&plane_m2_far, p);	/* Calculate the distance to
						 * the plane behind the the
						 * star.  */
	stuff_phot(p, &pp);
	...
	/*
	 * Finally sort it all out.  If n == 0, or if both s[0] and s[2] are
	 * negative then the photon did not hit the pillbox ksl 02jan
	 */

	if ((n == 0) || (ss[0] < 0 && ss[1] < 0))
		return (VERY_BIG);
	//The photon did not hit the pillbox
...
/*
 * Calculate the Roche potential at position indicated by the photon p_roche
 * and the length s
 */
	...
	double		s;
{
	...
	move_phot(&pp, s);	/* So now we have the actuaal position of the
				 * photon relative to the WD */
	...
	pp.x[0] -= geo.a;	/* Here we make pp refer to the positions
				 * w.r.t. the secondary */
...
/*
 * Calculate the half width of the Roche lobe at the position x along the x
 * axis in the plane of the orbit.  There is no real guarantee that this
 * would work if you were outside L2 or L3.
 */
	...
	double		x;
{
	...
	return (-rho);		/* This is because we are going to search for
				 * a minimun not a maximum */

}

/*
 * Find the maximum half width of the Roche lobe of the secondary.   This
 * routine uses the NR routine golden to find the point in x where the Roche
 * lobe is maximized.
 */
...
roche2_width_max()
{
	...
	/*
	 * golden returns the value of the function it is evaluating; xbest
	...
	return (-rmin);		/* Because "golden" is designed to find a
 ...
 * the slope of phie at s along the vector defined by p_roche.  The value of
...
/*
 * This is the function used by the Recipes routine rtsafe to lacate extrema
 * of the Roche potential.  Thus it can be used to find the L1 point, for
 * example.  Here value is the derivative of phi at s along the vector
 * defined by p_roche, and derivative is the second derivative
 */
