...
//	  	hydro_ptr[i * MAXHYDRO + j].v[2] = vphi;
  ...
  if (j_hydro_thetamax==0 || j_hydro_thetamax==i-1)
	...
	zdom[0].MDIM = zdom[0].mdim = ihydro_theta+1;
	}
  else
	{	
	...
	zdom[0].MDIM = zdom[0].mdim = ihydro_theta+2;
	}
...
/* Set a couple of last tags*/
	...
	zdom[0].coord_type=RTHETA; //At the moment we only deal with RTHETA - in the future we might want to do some clever stuff
	zdom[0].NDIM = zdom[0].ndim = ihydro_r+3; //We need an inner radial cell to bridge the star and the inside of the wind, and an outer cell
...
History:
	...
	15octq	NSH - swapped out lots of duplicated code into new subroutines
**************************************************************/
...
//	      thetamax = wind_z[j + 1] / RADIAN;
	      ...
	      thetamax = (2.0*wind_midz[j]-wind_z[j]) / RADIAN; //This from NSH fork...
